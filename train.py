...
def main():
    ...
    hps.train.device = 'cuda:'+ str(rank)
    ...
    # seed_everything() TODO
    train_dataset = TextAudioSpeakerLoader(hps.data,rank)
    ...
    train_loader = DataLoader(train_dataset, num_workers=24, shuffle=False, 
                            pin_memory=True, collate_fn=collate_fn, 
                            batch_sampler=train_sampler, persistent_workers=True,
                            prefetch_factor=4)
    ...
    net_d = MultiPeriodDiscriminator(hps.model.use_spectral_norm).to(hps.train.device)
    ...
    # net_g = net_g.to(hps.device)
    # net_d = net_d.to(hps.device)
    if net_dur_disc is not None:
    ...
    for epoch in range(epoch_str, hps.train.epochs + 1):
        if rank == 0:
            train_and_evaluate(rank, epoch, hps, [net_g, net_d, net_dur_disc], [optim_g, optim_d, optim_dur_disc], [scheduler_g, scheduler_d, scheduler_dur_disc], scaler, [train_loader, None], logger, [writer, writer_eval])
        else:
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers, tone, language, bert) in tqdm(enumerate(train_loader)):
        ...
        x, x_lengths = x.to(hps.train.device), x_lengths.to(hps.train.device)
        spec, spec_lengths = spec.to(hps.train.device), spec_lengths.to(hps.train.device)
        y, y_lengths = y.to(hps.train.device), y_lengths.to(hps.train.device)
        speakers = speakers.to(hps.train.device)
        tone = tone.to(hps.train.device)
        language = language.to(hps.train.device)
        bert = bert.to(hps.train.device)
