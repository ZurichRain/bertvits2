...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def get_audio_path(self,audiopath):
        ...
        wav_file = os.path.join('/data/hypertext/sharpwang/dataset/leidian/leidianid', audiopath+'.wav')
    ...
    def _filter(self):
        ...
        lengths = []
        skipped = 0
        for _id, language, text, phones, tone, word2ph in self.audiopaths_sid_text:
            audiopath = f'{_id}'
            # print(len(phones))
            if self.min_text_len <= len(phones) and len(phones) <= self.max_text_len:
                ...
                audiopath = self.get_audio_path(audiopath)
                # print(audiopath)
                try:
                    self.get_audio(audiopath)
                except:
                    continue
                audiopaths_sid_text_new.append([ori_audiopath, language, text, phones, tone, word2ph])
                ...
                skipped += 1
        print("skipped: ", skipped, ", total: ", len(self.audiopaths_sid_text), 'new: ', len(audiopaths_sid_text_new))
        self.audiopaths_sid_text = audiopaths_sid_text_new
        self.lengths = lengths

    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        # separate filename, speaker_id and text
        audiopath, language, text, phones, tone, word2ph = audiopath_sid_text
